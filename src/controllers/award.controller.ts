...
import AwardModel from "../database/schema/award.schema";
...
controller
  ...
  .post("/",authenticateToken, upload.single("image"), async (req: any, res) => {
    try {
      ...
      const newItem = new AwardModel({ ...req.body, image: imageUrl });
  ...
  .get("/", async (req: Request, res: Response) => {
    try {
      const items = await AwardModel.find();
  ...
  .get("/:id", async (req: Request, res: Response) => {
    try {
      const item = await AwardModel.findById(req.params.id);
      if (!item) {
  ...
  //     const updatedItem = await AwardModel.findByIdAndUpdate(req.params.id, { ...req.body, image: imageUrl }, { new: true });
  //     res.status(200).json({
  //       msg: "Update success",
  //       success: true,
  ...
  .delete("/:id",authenticateToken, async (req, res) => {
    try {
      ...
      const deletedItem = await AwardModel.findByIdAndDelete(req.params.id);
