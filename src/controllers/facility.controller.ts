...
import FacilityModel from "../database/schema/facility.schema";
...
controller
  ...
  .post("/",authenticateToken, upload.single("image"), async (req: any, res) => {
    try {
      ...
      const newItem = new FacilityModel({ ...req.body, image: imageUrl });
  ...
  .get("/", async (req: Request, res: Response) => {
    try {
      const items = await FacilityModel.find();
  ...
  .get("/:id", async (req: Request, res: Response) => {
    try {
      const item = await FacilityModel.findById(req.params.id);
      if (!item) {
  ...
  .patch("/:id",authenticateToken, upload.single("image"), async (req, res) => {
    try {
      ...
      const updatedItem = await FacilityModel.findByIdAndUpdate(req.params.id, { ...req.body, image: imageUrl }, { new: true });
  ...
  .delete("/:id",authenticateToken, async (req, res) => {
    try {
      ...
      const deletedItem = await FacilityModel.findByIdAndDelete(req.params.id);
