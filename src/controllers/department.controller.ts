...
import DepartmentModel from "../database/schema/department.schema";
...
controller
  ...
  .post("/", authenticateToken, upload.single("image"), async (req: any, res) => {
    try {
      ...
      const newItem = new DepartmentModel({ ...req.body, image: imageUrl });
  ...
  .get("/", async (req: Request, res: Response) => {
    try {
      const items = await DepartmentModel.find();
  ...
  .get("/get-name-and-slug-of-all-department", async (req: Request, res: Response) => {
    try {
      const items = await DepartmentModel.find().select("_id name slug");
      return res.status(200).json({ success: true, msg: "Get success", data: items });
    } catch (error: any) {
      return res.status(500).json({
        msg: "Something went wrong",
  ...
  .get("/less-data", async (req: Request, res: Response) => {
    try {
      const items = await DepartmentModel.find().select("_id name image");
      return res.status(200).json({ success: true, msg: "Get success", data: items });
    } catch (error: any) {
      return res.status(500).json({
        msg: "Something went wrong",
  ...
  //     const updatedItem = await DepartmentModel.findByIdAndUpdate(req.params.id, { ...req.body, image: imageUrl }, { new: true });
  //     res.status(200).json({
  //       msg: "Update success",
  //       success: true,
  ...
  .delete("/:id", authenticateToken, async (req, res) => {
    try {
      ...
      await DoctorDepartmentModel.deleteMany({ department: req.params.id });
      const deletedItem = await DepartmentModel.findByIdAndDelete(req.params.id);
