...
import AccrediationModel from "../database/schema/accrediation.schema";
...
controller
  ...
  .post("/",authenticateToken, upload.single("image"), async (req: any, res) => {
    try {
      ...
      const newItem = new AccrediationModel({ ...req.body, image: imageUrl });
  ...
  .get("/", async (req: Request, res: Response) => {
    try {
      const items = await AccrediationModel.find();
  ...
  .get("/:id", async (req: Request, res: Response) => {
    try {
      const item = await AccrediationModel.findById(req.params.id);
      if (!item) {
  ...
  //     const updatedItem = await AccrediationModel.findByIdAndUpdate(req.params.id, { ...req.body, image: imageUrl }, { new: true });
  //     res.status(200).json({
  //       msg: "Update success",
  //       success: true,
  ...
  .delete("/:id",authenticateToken, async (req, res) => {
    try {
      ...
      const deletedItem = await AccrediationModel.findByIdAndDelete(req.params.id);
