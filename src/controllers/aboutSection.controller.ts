...
import AboutSectionModel from "../database/schema/aboutSection.";
...
controller
  ...
  .post("/", authenticateToken, authenticateToken, upload.single("image"), async (req: any, res) => {
    try {
      const image = req.file;
      const imageUrl = await uploadSingleFile(image);

      const newItem = new AboutSectionModel({ ...req.body, image: imageUrl });
  ...
  .get("/", async (req: Request, res: Response) => {
    try {
      const items = await AboutSectionModel.find();
  ...
  .get("/:id", async (req: Request, res: Response) => {
    try {
      const item = await AboutSectionModel.findById(req.params.id);
      if (!item) {
  ...
  .patch("/:id", authenticateToken, upload.single("image"), async (req, res) => {
    try {
      ...
      const updatedItem = await AboutSectionModel.findByIdAndUpdate(req.params.id, { ...req.body, image: imageUrl }, { new: true });
  ...
  .delete("/:id", authenticateToken, async (req, res) => {
    try {
      ...
      const deletedItem = await AboutSectionModel.findByIdAndDelete(req.params.id);
