...
import EventModel from "../database/schema/event.schema";
...
controller
  ...
  .post("/",authenticateToken, upload.single("image"), async (req: any, res) => {
    try {
      ...
      const newItem = new EventModel({ ...req.body, image: imageUrl });
  ...
  .get("/:id", async (req: Request, res: Response) => {
    try {
      const item = await EventModel.findById(req.params.id);
      if (!item) {
  ...
  //     const updatedItem = await EventModel.findByIdAndUpdate(req.params.id, { ...req.body, image: imageUrl }, { new: true });
  //     res.status(200).json({
  //       msg: "Update success",
  //       success: true,
  ...
  .delete("/:id",authenticateToken, async (req, res) => {
    try {
      ...
      const deletedItem = await EventModel.findByIdAndDelete(req.params.id);
