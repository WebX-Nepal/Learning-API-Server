...
import FaqModel from "../database/schema/faq.schema";
...
controller
  .post("/",authenticateToken, async (req: Request, res: Response) => {
    try {
      const newItem = new FaqModel(req.body);
  ...
  .get("/:id", async (req: Request, res: Response) => {
    try {
      const item = await FaqModel.findById(req.params.id);
      if (!item) {
  ...
  .patch("/:id",authenticateToken, async (req: Request, res: Response) => {
    try {
      ...
      const updatedItem = await FaqModel.findByIdAndUpdate(req.params.id, req.body, { new: true });
      res.status(200).json({
        msg: "Update success",
        success: true,
        data: updatedItem,
  ...
  .delete("/:id",authenticateToken, async (req: Request, res: Response) => {
    try {
      const deletedItem = await FaqModel.findByIdAndDelete(req.params.id);
      if (!deletedItem) {
        return res.status(404).json({ success: false, msg: "Item not found" });
      }
