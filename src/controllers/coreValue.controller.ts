...
import CoreValueModel from "../database/schema/coreValue";
...
controller
  ...
  .post("/",authenticateToken, upload.single("image"), async (req: any, res) => {
    try {
      ...
      const newItem = new CoreValueModel({ ...req.body, image: imageUrl });
  ...
  .get("/", async (req: Request, res: Response) => {
    try {
      const items = await CoreValueModel.find();
  ...
  .get("/:id", async (req: Request, res: Response) => {
    try {
      const item = await CoreValueModel.findById(req.params.id);
      if (!item) {
  ...
  .patch("/:id",authenticateToken, upload.single("image"), async (req, res) => {
    try {
      ...
      const updatedItem = await CoreValueModel.findByIdAndUpdate(req.params.id, { ...req.body, image: imageUrl }, { new: true });
  ...
  .delete("/:id",authenticateToken, async (req, res) => {
    try {
      ...
      const deletedItem = await CoreValueModel.findByIdAndDelete(req.params.id);
