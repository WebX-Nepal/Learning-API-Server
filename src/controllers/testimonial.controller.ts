...
import TestimonialModel from "../database/schema/testimonial.schema";
...
controller
  .post("/", authenticateToken, async (req: Request, res: Response) => {
    try {
      const newItem = new TestimonialModel(req.body);
  ...
  .patch("/:id", authenticateToken, async (req: Request, res: Response) => {
    try {
      ...
      const updatedItem = await TestimonialModel.findByIdAndUpdate(req.params.id, req.body, { new: true });
      res.status(200).json({
        msg: "Update success",
        success: true,
        data: updatedItem,
  ...
  .delete("/:id", authenticateToken, async (req: Request, res: Response) => {
    try {
      const deletedItem = await TestimonialModel.findByIdAndDelete(req.params.id);
      if (!deletedItem) {
        return res.status(404).json({ success: false, msg: "Item not found" });
      }
