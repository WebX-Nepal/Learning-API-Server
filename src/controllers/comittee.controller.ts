...
import ComitteeModel from "../database/schema/comittee.schema";
...
controller
  ...
  .post("/",authenticateToken, upload.single("image"), async (req: any, res) => {
    try {
      ...
      const newItem = new ComitteeModel({ ...req.body, image: imageUrl });
  ...
  .get("/", async (req: Request, res: Response) => {
    try {
      const items = await ComitteeModel.find();
  ...
  .get("/:id", async (req: Request, res: Response) => {
    try {
      const item = await ComitteeModel.findById(req.params.id);
      if (!item) {
  ...
  //     const updatedItem = await ComitteeModel.findByIdAndUpdate(req.params.id, { ...req.body, image: imageUrl }, { new: true });
  //     res.status(200).json({
  //       msg: "Update success",
  //       success: true,
  ...
  .delete("/:id",authenticateToken, async (req, res) => {
    try {
      ...
      const deletedItem = await ComitteeModel.findByIdAndDelete(req.params.id);
