...
import FeatureModel from "../database/schema/feature.schema";
...
controller
  ...
  .post("/",authenticateToken, upload.single("image"), async (req: any, res) => {
    try {
      ...
      const newItem = new FeatureModel({ ...req.body, image: imageUrl });
  ...
  .get("/", async (req: Request, res: Response) => {
    try {
      const items = await FeatureModel.find();
  ...
  .get("/less-data", async (req: Request, res: Response) => {
    try {
      const items = await FeatureModel.find().select("_id name image");
      return res.status(200).json({ success: true, msg: "Get success", data: items });
    } catch (error: any) {
      return res.status(500).json({
        msg: "Something went wrong",
  ...
  .get("/:id", async (req: Request, res: Response) => {
    try {
      const item = await FeatureModel.findById(req.params.id);
      if (!item) {
  ...
  //     const updatedItem = await FeatureModel.findByIdAndUpdate(req.params.id, { ...req.body, image: imageUrl }, { new: true });
  //     res.status(200).json({
  //       msg: "Update success",
  //       success: true,
  ...
  .delete("/:id",authenticateToken, async (req, res) => {
    try {
      ...
      const deletedItem = await FeatureModel.findByIdAndDelete(req.params.id);
